...
public class SizeSetter
{
	...
	public SizeSetter ()
	{
		...
		if ((screenWidth/screenHeight) > 1.5)
		{
			...
			sizes[5] = (int) Math.round(screenWidth*0.22);// w side panels

			sizes[6] = (int) Math.round(screenHeight*0.6);// h done panel

			sizes[7] = (int) Math.round(screenHeight*0.5);// h player ships panel

			sizes[8] = (int) Math.round(screenWidth*0.4);// w winner player panels
			sizes[9] = (int) Math.round(screenHeight*0.1);// h winner player panels
			sizes[10] = (int) Math.round(screenHeight*0.35);// h winner main menu panel
		}
		...
		else
		{
			...
			sizes[5] = (int) Math.round(screenWidth*0.2);// w side panels

			sizes[6] = (int) Math.round(screenHeight*0.7);// h done panel

			sizes[7] = (int) Math.round(screenHeight*0.6);// h player ships panel

			sizes[8] = (int) Math.round(screenWidth*0.4);// w winner player panels
			sizes[9] = (int) Math.round(screenHeight*0.17);// h winner player panels
			sizes[10] = (int) Math.round(screenHeight*0.37);// h winner main menu panel
		}
